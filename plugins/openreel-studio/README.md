...
- 依赖线创建、更新和授权删除。
- 单节点运行、已有节点媒体上传、Codex 图片发布。
- 延迟能力的搜索、描述、普通执行和授权破坏性执行。

延迟目录提供 8 项复杂或低频能力：动态节点合同、节点复制、媒体历史切换、混合画布 patch、批量运行、等待终态、恢复画布快照、批量删除或恢复。搜索返回短摘要；描述返回精确 `input_schema`、安全类别、执行器名称和 `schema_ref`；执行阶段按 schema 校验参数。
